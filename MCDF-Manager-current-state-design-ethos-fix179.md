...
- Component-based Exchange rebuilds are allowed to produce a different outer MCDF container hash from the original uploaded package. The important validation for this rebuild mode is that every internal component blob matches its recorded hash and the resulting MCDF contains the same game payloads. Exact package-hash equality is only required for full archived-package downloads.


## fix179 — Exchange component rebuild semantics

The Exchange has two different download integrity modes:

- **Archived package**: the full original MCDF package is stored. Downloads must match the original package hash byte-for-byte.
- **Component rebuild**: verified internal component blobs are assembled into a new MCDF container. The rebuilt package is expected to work the same for the game, but the outer MCDF file does not need to be byte-identical to the uploaded source package.

The client should not block component-based Exchange downloads merely because the rebuilt container hash differs from the original package hash. It should verify every component blob by hash, write the rebuilt MCDF, and show a quiet note that the rebuilt container hash differs while the internal game files are preserved.
